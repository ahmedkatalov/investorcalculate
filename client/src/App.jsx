...
export default function App() {
    ...
    fetchPayouts().then((data) => {
      if (Array.isArray(data)) {
        setPayouts(
          data.map((p) => ({
            ...p,
            periodMonth: p.periodMonth || p.period_month || null,
            reinvest: p.reinvest ?? false,
  ...
  const handlePercentChange = (id, value) => {
    ...
    // разрешаем только цифры, запятую и точку визуально
  ...
  const handleConfirmDelete = async () => {
    ...
    // удаляем на бэке инвестора и его выплаты (каскадом)
    await deleteInvestorApi(invId);

    // удаляем инвестора из стейта
    ...
    // чистим выплаты из стейта,
    // чтобы месяцы без операций полностью исчезали
  ...
  // для каждого месяца считаем максимальное количество операций по любому инвестору
  // и создаём столько колонок: ноябрь, ноябрь, дек, дек, дек и т.д.
  // Правило: если в месяце нет ни одной выплаты ни у одного инвестора — этого месяца тут нет.
  ...
  const visibleMonthSlots = useMemo(
    () =>
      monthSlots.slice(
        monthOffset,
        monthOffset + MAX_VISIBLE_MONTH_SLOTS
      ),
  ...
  const handlePrevMonths = () => {
    setMonthOffset((prev) =>
      Math.max(0, prev - MAX_VISIBLE_MONTH_SLOTS)
    );
  };
  ...
  const handleConfirmPayout = async () => {
    ...
    const payoutAmount = Math.round((capitalBefore * percent) / 100);

    setIsSavingPayout(true);

    try {
      try {
        await createPayout(inv.id, monthKey, percent);
      } catch {
        // молча
      }
      ...
      setPayouts((prev) => [
        ...prev,
        {
          id: `${Date.now()}-${inv.id}-${Math.random()}`,
          investorId: inv.id,
...
<div
  className="
    ...
    h-[calc(100vh-120px)]   /* 🔥 Автоматически занимает всё окно */
    overflow-y-auto         /* 🔥 Тело скроллится */
  "
>
...
<thead>
  <tr className="text-slate-300 bg-slate-700">
    ...
    {visibleMonthSlots.map((slot, idx) => {
      const label = new Date(slot.month).toLocaleDateString("ru-RU", {
        month: "short",
        year: "2-digit",
      });
              ...
              {investors.map((inv, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Капитал сейчас (авто) + кнопка снятия */}
                    <td className="py-2 px-4 border-r border-slate-700/50">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {/* Иконка — стрелка вниз с кошельком */}
                          <svg
                            width="18"
                            height="18"
                            viewBox="0 0 24 24"
...
<td className="py-2 px-4 border-r border-slate-700/50 text-center min-w-[90px]">
  ...
  {draft > 0 ? fmt(draft) + " ₽" : "—"}
                    ...
                    {/* История по месяцам: каждая операция — отдельная колонка (с пагинацией) */}
                    {visibleMonthSlots.map((slot, idx) => {
...
>
  <span
    className={`whitespace-nowrap ${
      isNegative ? "text-red-400 font-semibold" : "text-slate-100"
    }`}
  >
    {isNegative ? `- ${fmt(abs)} ₽` : `${fmt(abs)} ₽`}
  </span>
</td>
